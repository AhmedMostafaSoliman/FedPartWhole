...
class CCNet(pl.LightningModule):
    def __init__(self,
        FLAGS,
        *,
        ...
        self.classification_head_from_2nd_level = nn.Sequential(
            nn.LayerNorm(FLAGS.patch_dim),
            nn.Dropout(p=0.5),
            Rearrange('b n d -> b (n d)'),
            nn.LayerNorm(self.num_patches * FLAGS.patch_dim),
        ...
        self.classification_head_from_1st_level = nn.Sequential(
            nn.LayerNorm(FLAGS.patch_dim),
            nn.Dropout(p=0.5),
            Rearrange('b n d -> b (n d)'),
            nn.LayerNorm(self.num_patches * FLAGS.patch_dim),
    ...
    def forward(self, img, levels = None):
        ...
        second_level = all_levels[self.FLAGS.denoise_iter, :, :, -2]
        first_level =   all_levels[self.FLAGS.denoise_iter, :, :, -3]

        #top_level = F.normalize(top_level, dim=1)
        #second_level = F.normalize(second_level, dim=1)
        #first_level = F.normalize(first_level, dim=1)

        encoded_levels_out = all_levels[-1, :, :, :, :]
        if self.FLAGS.levels_encoder:
            encoded_levels_out = self.levels_encoder(all_levels[-1, :, :, :, :])
        if self.FLAGS.supervise:
            out1 = self.classification_head_from_last_level(top_level)
            out2 = self.classification_head_from_2nd_level(second_level)
            out3 = self.classification_head_from_1st_level(first_level)
            out = torch.mean(torch.stack((out1, out2, out3)), dim=0)
            #out = torch.mean(torch.stack((out1, out2)), dim=0)
            return out1, encoded_levels_out
        else:
