...
class PACS_SingleDomain():
    def __init__(self, root_path=pacs_path, domain_name='p', split='total', train_transform=None):
        ...
        imgs, labels, embeds, indices = PACS_SingleDomain.read_txt(self.split_file, self.root_path)
    ...
    def read_txt(txt_path, root_path):
        ...
        for line_txt in txt_component:
            ...
            embeds.append(os.path.join(root_path, 'emb', line_txt[0].replace('.jpg', '.pkl').replace('.png', '.pkl')))
            labels.append(int(line_txt[1]) - 1)
            indices.append(int(line_txt[2]))
        return imgs, labels, embeds, indices
